...
class KNNClassifier:
    ...
    def __init__(self, n_neighbors, metric='euclidean') -> None:
        assert metric in self.supported_metrics, f"Unsupported metric: {metric}. Supported metrics are: {self.supported_metrics}"
    ...
    def aggregate_n_neighbours(self, n_labels: list[Union[float, str]]) -> Union[int, str]:
...
class KNNRegressor(KNNClassifier):
    ...
    def __init__(self, n_neighbors, metric='minkowski', aggregate='mean'):
        ...
        assert aggregate in self.supported_aggregations, f"Unsupported aggregation: {aggregate}. Supported aggregations are: {self.supported_aggregations}"
